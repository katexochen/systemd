...
static int run_virtual_machine(int kvm_device_fd, int vhost_device_fd) {
        ...
        if (arg_confidential_computing == CC_AMD_SEV_SNP) {
                ...
                if (arg_linux) {
                        ...
                        r = qemu_config_key(config_file, "kernel-hashes", "on");
                        if (r < 0)
                                return r;
                }
        }
...
static int verify_arguments(void) {
        ...
        if (arg_confidential_computing == CC_AMD_SEV_SNP) {
                ...
                /* TODO: --image= contents are loaded by firmware after the launch measurement and
                 * are not covered by it. The guest must verify the rootfs itself (e.g. dm-verity
                 * rooted in the measured kernel cmdline). Consider rejecting --image= without
                 * --linux= once we have a clear measured-rootfs story. */
        }
